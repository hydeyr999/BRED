...
def run_ghostbuster(args,test_df= None,flag=None):
    ...
    if args.mode == 'train':
        ...
        if args.run_logprobs:
            ...
            if args.if_save:
                train_df['logprob_small'] = logprobs_small
                train_df['logprob_large'] = logprobs_large
                print(train_df)
                if args.task == 'op-co' or args.task == 'llm-co':
                    train_df.to_json(
                        f'./detectors/ling-based/logprobs/v1/{args.task}/{args.dataset}_train_n{args.frac}.json',orient="records", lines=True)
                else:
                    train_df.to_json(
                        f'./detectors/ling-based/logprobs/v1/{args.task}/test/{args.dataset}_{name_small}_{name_large}_multi{args.multilen}.json',
        ...
        model_save_dir = f'./detectors/ling-based/classifier/v1'
        if args.imbddata:
            joblib.dump(classifier, f'{model_save_dir}/otherdata/gb_{args.dataset}_n{args.n_sample}_{args.translate}.pkl')
        elif args.task == 'op-co' or args.task == 'llm-co':
            joblib.dump(classifier,f'{model_save_dir}/{args.task}/gb_{args.dataset}_n{args.frac}.pkl')
        else:
        ...
        if args.if_save:
            test_df['logprob_small'] = logprobs_small
            test_df['logprob_large'] = logprobs_large
            print(test_df)

            if args.task == 'op-co' or args.task == 'llm-co':
                test_df.to_json(f'./detectors/ling-based/logprobs/v1/{args.task}/{args.dataset}_test_n{args.frac}_{flag}.json',orient="records", lines=True)
            else:
                test_df.to_json(f'./detectors/ling-based/logprobs/v1/{args.task}/test/{args.dataset}_{name_small}_{name_large}_multi{args.multilen}.json',
                                        orient="records", lines=True)
            return
        ...
        results = {'predictions': predictions.tolist(),
                   ...
                   'loss': 1 - pr_auc}
        if args.imbddata:
            res_save_dir = f'./detectors/ling-based/results/otherdata'
            if not os.path.exists(res_save_dir):
                os.makedirs(res_save_dir)
            with open(f'{res_save_dir}/gb_{args.dataset}_n{args.n_sample}_multi{args.multilen}_{args.translate}_{args.language}.json', 'w') as fout:
                json.dump(results, fout)
                print(f'Results written into {res_save_dir}/gb_{args.dataset}_n{args.n_sample}_multi{args.multilen}_{args.translate}_{args.language}.json')
        else:
            res_save_dir = f'./detectors/ling-based/results/v1/{args.task}'
            if not os.path.exists(res_save_dir):
                os.makedirs(res_save_dir)
            if args.task == 'op-co':
                with open(f'{res_save_dir}/gb_{args.dataset}_{args.op2}_n{args.frac}_{args.flag}.json','w') as fout:
                    json.dump(results, fout)
                    print(f'Results written into {res_save_dir}/gb_{args.dataset}_{args.op2}_n{args.frac}_{args.flag}.json')
            elif args.task == 'llm-co':
                with open(f'{res_save_dir}/gb_{args.dataset}_{args.model2}_n{args.frac}_{args.flag}.json','w') as fout:
                    json.dump(results, fout)
                    print(f'Results written into {res_save_dir}/gb_{args.dataset}_{args.model2}_n{args.frac}_{args.flag}.json')
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path_small', type=str, default='../detectbt/models/tinyllama')
    parser.add_argument('--model_path_large', type=str, default='../detectbt/models/llama2_7b')
    ...
    if args.task == 'op-co' or args.task == 'llm-co':
        if args.mode !='train' and args.run_logprobs:
            ...
            run_ghostbuster(args,test_df1,flag='1')
            run_ghostbuster(args,test_df2,flag='2')
        else:
