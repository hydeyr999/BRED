...
def run_train(args):
    ...
    if args.run_feats:
        ...
        if args.save_feats:
            train_df['gltr_feats'] = gltr_feats.tolist()
            train_df['ppl_feats'] = ppl_feats.tolist()
            if args.task == 'op-co' or args.task == 'llm-co':
                train_df.to_json(f'./detectors/ling-based/lingfeatures/v1/{args.task}/{args.dataset}_gltrppl_train_n{args.frac}.json',
                    orient='records', lines=True)
            else:
                train_df.to_json(f'./detectors/ling-based/lingfeatures/v1/{args.task}/test/{args.dataset}_gltrppl_{name_small}_{name_large}_multi{args.multilen}.json'
                    , orient="records", lines=True)
            return
    else:
        if args.func == 'ppl':
    ...
    if args.task == 'op-co' or args.task == 'llm-co':
        joblib.dump(classifier, f'{model_save_dir}/{args.task}/{args.func}_{args.dataset}_n{args.frac}.pkl')
    else:
        joblib.dump(classifier, f'{model_save_dir}/{args.task}/{args.func}_{args.dataset}_n{args.n_sample}.pkl')

def run_test(args,test_df=None,flag=None):
    ...
    if args.run_feats:
        ...
        if args.save_feats:
            test_df['gltr_feats'] = gltr_feats.tolist()
            test_df['ppl_feats'] = ppl_feats.tolist()
            if args.task == 'op-co' or args.task == 'llm-co':
                test_df.to_json(f'./detectors/ling-based/lingfeatures/v1/{args.task}/{args.dataset}_gltrppl_test_n{args.frac}_{flag}.json',orient='records',lines=True)
            else:
                test_df.to_json(f'./detectors/ling-based/lingfeatures/v1/{args.task}/test/{args.dataset}_gltrppl_{name_small}_{name_large}_multi{args.multilen}.json'
                        , orient="records", lines=True)
        return
    else:
        if args.func == 'ppl':
    ...
    if args.imbddata:
        ...
        with open(f'{res_save_dir}/{args.func}_{args.dataset}_n{args.n_sample}_multi{args.multilen}_{args.translate}_{args.language}.json', 'w') as fout:
            json.dump(results, fout)
            print(f'Results written into {res_save_dir}/{args.func}_{args.dataset}_n{args.n_sample}_multi{args.multilen}_{args.translate}_{args.language}.json')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_small', type=str, default='../detectbt/models/tinyllama')
    parser.add_argument('--model_large', type=str, default='../detectbt/models/llama2_7b')
    ...
    if args.mode == 'train':
        ...
        if args.task == 'op-co' or args.task == 'llm-co':
            if args.run_feats:
                test_df1,test_df2 = get_gltr_df(args)
                test_df1 = test_df1.dropna().reset_index(drop=True)
                test_df2 = test_df2.dropna().reset_index(drop=True)
                run_test(args,test_df1,flag='1')
                run_test(args,test_df2,flag='2')
            else:
                run_test(args)
        else:
            run_test(args)