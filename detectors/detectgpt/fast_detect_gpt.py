...
def experiment(args,data,flag=None):
    ...
    if args.task == 'op-co':
        with open(f'{detect_save_dir}/fast_{args.dataset}_{args.op2}_n{args.frac}_{flag}.json', 'wb') as fout:
            pickle.dump(results, fout)
            print(f'Results written into {detect_save_dir}/fast_{args.dataset}_{args.op2}_n{args.frac}_{flag}.json')
    elif args.task == 'llm-co':
        with open(f'{detect_save_dir}/fast_{args.dataset}_{args.model2}_n{args.frac}_{flag}.json', 'wb') as fout:
            pickle.dump(results, fout)
            print(f'Results written into {detect_save_dir}/fast_{args.dataset}_{args.model2}_n{args.frac}_{flag}.json')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cache_dir', type=str, default="../detectbt/models/")
    ...
    if args.task == 'op-co' or args.task == 'llm-co':
        ...
        experiment(args,test_df1,flag='1')
        experiment(args,test_df2,flag='2')
    else:
        test_df = get_df(args).dropna().reset_index(drop=True)
        print(test_df)
        experiment(args,test_df)
