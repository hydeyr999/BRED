...
def run(args,df,flag=None):
    ...
    if args.task == 'op-co':
        with open(f'{detect_save_dir}/detectgpt_{args.dataset}_{args.op2}_n{args.frac}_{flag}.json', 'w') as f:
            json.dump(outputs, f, default=custom_serializer)
            print(f'Results written into {detect_save_dir}/baselines_{args.dataset}_{args.op2}_n{args.frac}_{flag}.json')
    elif args.task == 'llm-co':
        with open(f'{detect_save_dir}/detectgpt_{args.dataset}_{args.model2}_n{args.frac}_{flag}.json', 'w') as f:
            json.dump(outputs, f, default=custom_serializer)
            print(f'Results written into {detect_save_dir}/baselines_{args.dataset}_{args.model2}_n{args.frac}_{flag}.json')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_model', type=str, default='../detectbt/models/gpt2-medium', help="Base model path.")
    parser.add_argument('--mask_model', type=str, default='../detectbt/models/t5-base', help="Mask model path.")
    ...
    if args.task == 'op-co' or args.task == 'llm-co':
        ...
        data, data_label = generate_data(args, test_df1, 'text')
        run(args,test_df1,'1')

        data, data_label = generate_data(args, test_df2, 'text')
        run(args,test_df2,'2')
    else:
        test_df = get_df(args).dropna().reset_index(drop=True)
        print(test_df)
        data, data_label = generate_data(args, test_df, 'text')
        run(args,test_df)
