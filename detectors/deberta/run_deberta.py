...
def train_deberta(args,train_df):
    model_path = '../detectbt/models/deberta-v3-large'
    ...
    if args.imbddata:
        ...
        torch.save(model_weights,
                       f'{bert_save_dir}/deberta_{args.dataset}_ep{args.epochs}_thres{args.threshold}_n{args.n_sample}_{args.translate}')
    elif args.task == 'op-co' or args.task == 'llm-co':
        bert_save_dir = f'./detectors/deberta/weights/v1/{args.task}'
        torch.save(model_weights,f'{bert_save_dir}/deberta_{args.dataset}_ep{args.epochs}_thres{args.threshold}_n{args.frac}')
    else:
        bert_save_dir = f'./detectors/deberta/weights/v1/{args.task}'
...
def get_deberta_test(args,test_df,flag=None):
    ...
    if args.imbddata:
        ...
        bert_save_path = f'{bert_save_dir}/deberta_{args.dataset}_ep{args.epochs}_thres{args.threshold}_multi{args.multilen}_n{args.n_sample}_{args.translate}.csv'
    elif args.task == 'op-co':
        bert_save_dir = f'./detectors/deberta/results/v1/{args.task}'
        bert_save_path = f'{bert_save_dir}/deberta_{args.dataset}_{args.op2}_n{args.frac}_{flag}_ep{args.epochs}_thres{args.threshold}.csv'
    elif args.task == 'llm-co':
        bert_save_dir = f'./detectors/deberta/results/v1/{args.task}'
        bert_save_path = f'{bert_save_dir}/deberta_{args.dataset}_{args.model2}_n{args.frac}_{flag}_ep{args.epochs}_thres{args.threshold}.csv'
    else:
        bert_save_dir = f'./detectors/deberta/results/v1/{args.task}'
