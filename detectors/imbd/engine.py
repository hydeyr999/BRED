...
def fine_tune_ours(model, data, DEVICE, ckpt_dir='./ckpt', args=None):
    ...
    writer = SummaryWriter(log_dir=f"./detectors/imbd/logs/v1/{args.task_name}/{args.train_dataset}_n{args.n_sample}_ep{args.epochs}_lr_{args.lr}_beta_{args.beta}_a_{args.a}_{current_time}/train_ai_detection")
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm.tqdm(train_loader, desc=f"Fine-tuning: {epoch} epoch"):
            try:
                ...
                epoch_losses.append(loss.item())
                i += 1
            except torch.OutOfMemoryError as e:  # 捕获显存不足异常
        ...
        if args.threshold is not None and epoch_avg_loss < args.threshold:
            print(f"\nLoss dropped below threshold {args.threshold}, stopping training early.")
        ...
        if ((epoch+1) % args.val_freq)==0: # eval and save model every val_freq epoch
            ...
            valwriter = SummaryWriter(log_dir=f"./detectors/imbd/logs/v1/{args.task_name}/{args.eval_dataset}_n{args.n_sample}_ep{args.epochs}_lr_{args.lr}_beta_{args.beta}_a_{args.a}_{current_time}/val_{dataset_name}")
        ...
        if args.SPOtrained:
            if args.task_name == 'ai_detection_500':
                output_path = os.path.join(args.output_file,
                                           f"{args.task_name}/{dataset_name}_ep{args.epochs}_n{args.datanum}_multi{args.cut_len}.json")
            else:
                output_path = os.path.join(args.output_file,
                                           f"{args.task_name}/{dataset_name}_ep{args.epochs}_n{args.datanum}_multi{args.cut_len}.json")
        else:
