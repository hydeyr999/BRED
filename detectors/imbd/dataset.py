...
class CustomDataset_rewrite(Dataset):
    def __init__(self, args,data_json_dir,mode='train',flag=None):
        if mode == 'train':
            if args.task_name == "ai_detection_500":
                self.data_dir = './data/otherdata/imbd_sample.csv'
                print(self.data_dir)
                data_df = pd.read_csv(self.data_dir)
                data_df = get_sample(data_df,args.n_sample)
                print(data_df)
                self.data = self.process_data(data_df)
            elif args.task_name == 'llm-co' or args.task_name == 'op-co':
                data_path = f'./data/v1/{args.task_name}/train/{data_json_dir}_sample_train_n{args.frac}.json'
                print(data_path)
                df = pd.read_json(data_path, lines=True)
                print(df)
                original = df['human'].values.tolist()
                rewritten = df['LLM_1'].values.tolist()
                data_df = pd.DataFrame({'original': original, 'rewritten': rewritten}).dropna().reset_index(drop=True)
                self.data = self.process_data(data_df)
            else:
                datasets_dict = {
                ...
                if args.task_name == 'thinking':
                    datasets = ['llama', 'deepseek', 'deepseek_thinking', 'gpt_4o', 'gpt_5', 'gpt_5_thinking', 'Qwen', 'Qwen_thinking']
                    train_dir = f'./data/v1/{args.task_name}'
                    eval_dataset = data_json_dir.split('/')[-1].split('_')[0]
                    paths = [f'{train_dir}/{x}_sample.csv' for x in datasets if eval_dataset not in x]
                    print(paths)
                else:
            ...
            if args.task_name == 'op-co' or args.task_name == 'llm-co':
                test_dir = f'./data/v1/{args.task_name}/test/{data_json_dir}_sample_test_n{args.frac}.json'
                print(test_dir)
                df = pd.read_json(test_dir, lines=True)
                if args.task_name == 'op-co':
                    print(args.op2)
                    df = df[df['op2'] == args.op2].reset_index(drop=True)
                elif args.task_name == 'llm-co':
                    print(args.model2)
                ...
                if args.cut_len > 0:
                    test_dir = f'./data/v1/multilen/len_{args.cut_len}/{args.task_name}/{args.eval_dataset}_len_{args.cut_len}.csv'
                    print(test_dir)
                    data_df = pd.read_csv(test_dir)
                    data_df['text'] = data_df[f'text_{args.cut_len}']
                    print(data_df)
                else:
                    test_dir = f'./data/v1/{args.task_name}'
