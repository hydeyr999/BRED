...
def get_ref_data(model,tokenizer,df):
...
def run(args):
    ...
    if (args.task == 'op-co' or args.task == 'llm-co') and args.mode != 'train':
        df1,df2 = prompt.run_get_prompt(model,tokenizer,args)
        df1 = get_ref_data(model,tokenizer,df1)
        df2 = get_ref_data(model,tokenizer,df2)
        return df1,df2
    else:
