...
def get_dpic_train(model,tokenizer,df,device='cuda:0',batch_size=4,num_epoch=3,learning_rate = 0.00001,max_len = 512,threshold=0):
    model.to(device)
...
def save_model(args,model_weights):
    if args.imbddata:
        save_path = os.path.join(args.model_save_dir, f'otherdata/dpic_{args.dataset}_ep{args.epochs}_thres{args.threshold}_n{args.n_sample}.pth')
    else:
        save_path = os.path.join(args.model_save_dir, f'{args.task}/dpic_{args.dataset}_ep{args.epochs}_thres{args.threshold}_n{args.n_sample}.pth')
...
def run_dpic_train(args):
    if args.run_generate:
        ...
        if args.imbddata:
            if args.translate:
                df.to_json(f'{args.data_save_dir}/otherdata/{args.dataset}_35_translate_sample.json', orient='records',lines=True)
            else:
                df.to_json(f'{args.data_save_dir}/otherdata/{args.dataset}_sample.json', orient='records',lines=True)
        else:
