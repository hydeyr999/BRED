...
def get_baselines(args,model,tokenizer,test_df,flag=None):
    ...
    for name in criterion_fns:
        ...
        print(f'{name}_roc_auc: {roc_auc:.4f},{name}_pr_auc: {pr_auc:.4f}\n')
        ...
        detect_save_dir = f'./detectors/baselines/results/v1/{args.task}'
        if not os.path.exists(detect_save_dir):
            os.makedirs(detect_save_dir)
        if args.task == 'op-co':
            with open(f'{detect_save_dir}/{name}_{args.dataset}_{args.op2}_n{args.frac}_{flag}.json', 'w') as fout:
                json.dump(results, fout)
                print(f'Results written into {detect_save_dir}/{name}_{args.dataset}_{args.op2}_n{args.frac}_{flag}.json')
        elif args.task == 'llm-co':
            with open(f'{detect_save_dir}/baselines_{args.dataset}_{args.model2}_n{args.frac}_{flag}.json', 'w') as fout:
                json.dump(results, fout)
                print(f'Results written into {detect_save_dir}/{name}_{args.dataset}_{args.model2}_n{args.frac}_{flag}.json')
        else:
            with open(f'{detect_save_dir}/baselines_{args.dataset}_multi{args.multilen}.json', 'w') as fout:
                json.dump(results, fout)
                print(f'Results written into {detect_save_dir}/{name}_{args.dataset}_multi{args.multilen}.json')
...
if __name__ == '__main__':
    ...
    if args.task == 'op-co' or args.task == 'llm-co':
        ...
        get_baselines(args, model, tokenizer, test_df1,flag='1')
        get_baselines(args, model, tokenizer, test_df2,flag='2')
    else:
