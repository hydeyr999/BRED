...
from transformers import AutoTokenizer, tokenization_utils, RobertaTokenizer, BertTokenizer, \
    RobertaForSequenceClassification, RobertaModel, AutoModelForSequenceClassification, XLNetTokenizer, \
    XLNetForSequenceClassification, DistilBertTokenizer, DistilBertForSequenceClassification, \
...
def run(max_epochs=None,
    ...
    for epoch in epoch_loop:
        ...
        # if kwargs['threshold'] is not None and combined_metrics["train/loss"] < kwargs['threshold']:
        #     if rank == 0:
        #         print(f"Validation loss {combined_metrics['train/loss']:.4f} < threshold {kwargs['threshold']}, stopping early.")
        ...
        if rank == 0:
            ...
            tokenizer.save_pretrained(os.path.join(logdir, f"complete-{max_epochs}"))
            if hasattr(model, 'save_pretrained'):
                model.save_pretrained(os.path.join(logdir, f"complete-{max_epochs}"))
            else:
                model.module.save_pretrained(os.path.join(logdir, f"complete-{max_epochs}"))
...
if __name__ == '__main__':
    ...
    if args.log_dir is None:# train_summary/train_config/Aug+PUconfig
        ...
        if args.imbddata:
            args.log_dir = f'./detectors/mpu/results/v1/otherdata/mpu_HC3_ep{args.max_epochs}_thres{args.threshold}_n{args.n_sample}_{args.translate}'
        elif args.task == 'op-co' or args.task == 'llm-co':
            args.log_dir = f'./detectors/mpu/results/v1/{args.task}/mpu_{args.dataset}_n{args.frac}_ep{args.max_epochs}_thres{args.threshold}'
        else:
            args.log_dir = f'./detectors/mpu/results/v1/{args.task}/mpu_{args.dataset}_ep{args.max_epochs}_thres{args.threshold}_n{args.n_sample}'
