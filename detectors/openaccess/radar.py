...
def get_detect(args,df,flag=None):
    ...
    if args.task == 'op-co':
        results_df.to_csv(f'./detectors/opensource/results/v1/{args.task}/radar_{args.dataset}_{args.op2}_n{args.frac}_{flag}.csv',index=False)
    elif args.task == 'llm-co':
        results_df.to_csv(f'./detectors/opensource/results/v1/{args.task}/radar_{args.dataset}_{args.model2}_n{args.frac}_{flag}.csv',index=False)
    else:
        results_df.to_csv(f'./detectors/opensource/results/v1/{args.task}/radar_{args.dataset}_multi{args.multilen}.csv', index=False)
