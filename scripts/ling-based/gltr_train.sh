...
for func in ${funcs[@]}; do
  for domain in ${domains[@]}; do
    echo "Training on $domain, n_sample=$n_sample, func=$func"
...
#    echo "Training on $model, n_sample=$n_sample, func=$func"
#    python detectors/ling-based/gltrppl.py --task cross-model --dataset $model \
#    --n_sample $n_sample --device $device --mode train --func $func
#  done

  for operation in ${operations[@]}; do
    echo "Training on $operation, n_sample=$n_sample, func=$func"
