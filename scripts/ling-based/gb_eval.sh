...
for domain in ${domains[@]}; do
  ...
  --dataset $domain \
  --mode test \
  --classifier ./detectors/ling-based/classifier/v1/cross-domain/n${n_sample}/gb_$domain\_n${n_sample}.pkl \
  --n_sample $n_sample \
  --multilen $multilen \
  --device $device
done
...
for operation in ${operations[@]}; do
  ...
  --dataset $operation \
  --mode test \
  --classifier ./detectors/ling-based/classifier/v1/cross-operation/n${n_sample}/gb_$operation\_n${n_sample}.pkl \
  --n_sample $n_sample \
  --multilen $multilen \
  --device $device
done