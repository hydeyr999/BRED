...
#python detectors/deberta/run_deberta.py --dataset HC3 --mode train --device $device \
...
#  --deberta_model ./detectors/deberta/weights/otherdata/deberta_imbd_ep$epochs\_thres$threshold\_n$n_sample \
...
#  --deberta_model ./detectors/deberta/weights/otherdata/deberta_imbd_ep$epochs\_thres$threshold\_n$n_sample\_False \
#  --device $device --epochs $epochs --threshold $threshold --n_sample $n_sample --multilen $multilen --imbddata True
#done
...
for operation in ${operations[@]}; do
  echo "Testing operation: $operation, epochs: $epochs, threshold: $threshold, n_sample: $n_sample"
  python detectors/deberta/run_deberta.py --task cross-operation --dataset $operation --mode test \
  --deberta_model ./detectors/deberta/weights/otherdata/deberta_HC3_ep$epochs\_thres$threshold\_n$n_sample\_True \
