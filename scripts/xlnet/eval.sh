...
for domain in ${domains[@]}; do
  ...
  python detectors/xlnet/run_xlnet.py \
  --task cross-domain \
  --dataset $domain --mode test \
  --xlnet_model ./detectors/xlnet/weights/v1/cross-domain/n${n_sample}/xlnet_$domain\_ep$epochs\_thres$threshold\_n$n_sample \
  --device $device \
  --epochs $epochs \
  --threshold $threshold \
  --n_sample $n_sample \
...
for operation in ${operations[@]}; do
  ...
  --dataset $operation \
  --mode test \
  --xlnet_model ./detectors/xlnet/weights/v1/cross-operation/n${n_sample}/xlnet_$operation\_ep$epochs\_thres$threshold\_n$n_sample \
  --device $device \
  --epochs $epochs \
  --threshold $threshold \
  --n_sample $n_sample \
